...
public class AiTradeManagementEngine {
    ...
    public void registerTrade(Trade trade, AiTradeDecision decision) {
        ...
        persistPosition(pos);
        // FIX (found via direct user report: "Daily P&L" and "Trades
        // Today" on the dashboard always showed zero regardless of
        // real trading activity). Confirmed root cause: CircuitBreaker
        // Service.recordTradeEntered() was defined and ready to use,
        // but never actually called anywhere in the entire codebase -
        // a structural gap, not a display bug. Wired in here, at the
        // exact point a position genuinely opens.
    ...
    private void actuallyClosePosition(String symbol, double exitPrice, String reason) {
        ...
        // FIX (found via direct user report: "Daily P&L" always showed
        // zero regardless of real trading activity). Confirmed root
        // cause: CircuitBreakerService.recordPnl() was defined and
        // ready to use, but never actually called anywhere in the
        // entire codebase - meaning the circuit breaker's own
        // protective function (halting trading after a real daily/
        // weekly/monthly loss) has likely never been functional either,
        // not just the dashboard display. Wired in here, at the exact
        // point real, realised P&L is finalized.
        circuitBreaker.recordPnl(bd(totalPnl, 2));
