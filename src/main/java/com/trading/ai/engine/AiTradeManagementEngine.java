...
public class AiTradeManagementEngine {
    ...
    public void reconcileFromDatabase() {
        try {
            List<Map<String, Object>> rows = jdbc.queryForList(
                    "SELECT * FROM ai_open_positions WHERE trade_date = ?", LocalDate.now(ZoneId.of("Asia/Kolkata")));
            if (rows.isEmpty()) {
            ...
            log.info("[AI-MGMT] [OK] Reconciliation complete - {} position(s) restored", rows.size());
        } catch (Exception e) {
    ...
    private void actuallyClosePosition(String symbol, double exitPrice, String reason) {
        ...
        log.info("[AI-MGMT] CLOSED: {} {} @ {} | R={} P&L=Rs.{} reason={}",
    ...
    private void onLiveExitRejected(String symbol, String statusMessage) {
        AiPosition pos = openPositions.get(symbol);
        if (pos == null) return;
        log.error("[AI-MGMT] [WARN] LIVE exit order was rejected/cancelled for {} - position " +
    ...
    public void dailyReset() {
        ...
        try {
            jdbc.update("DELETE FROM ai_open_positions WHERE trade_date < ?", LocalDate.now(ZoneId.of("Asia/Kolkata")));
        } catch (Exception e) {
