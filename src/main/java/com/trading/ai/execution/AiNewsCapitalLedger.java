 ...
 *
 * FIX (per-strategy capital, requested after going live with ₹10,000 split
    ...
    private synchronized void ensureRowExists(String strategyName) {
        try {
            Integer exists = jdbc.queryForObject(
                    ...
                    Integer.class, LocalDate.now(), strategyName);
            if (exists == null || exists == 0) {
                ...
                jdbc.update("""
                    ...
                    VALUES (?, ?, ?, 0, 0, 0, 0, 0, ?)
                    """,
                        LocalDate.now(), strategyName, base, java.sql.Timestamp.from(Instant.now()));
                log.info("[AI-LEDGER] Initialised today's ledger for {} with starting capital ₹{}",
                        strategyName, base);
            }
        } catch (Exception e) {
    ...
    public synchronized boolean setStartingCapital(String strategyName, BigDecimal capital) {
        try {
            ...
            jdbc.update("""
                ...
                """,
                    capital, java.sql.Timestamp.from(Instant.now()), LocalDate.now(), strategyName);
            log.info("[AI-LEDGER] {} capital set to ₹{} via UI", strategyName, capital);
            return true;
        } catch (Exception e) {
    ...
    public synchronized void debitMargin(String symbol, String strategyName,
        ...
        try {
            ...
            jdbc.update("""
                ...
                WHERE trade_date = ? AND strategy_name = ?
                """,
                    marginRequired, java.sql.Timestamp.from(Instant.now()), LocalDate.now(), strategyName);
            recordEntry(symbol, strategyName, "MARGIN_DEBIT", marginRequired.negate(),
                    "Entry margin reserved");
            log.info("[AI-LEDGER] Margin debited: {} ₹{} for {} (strategy={})",
        ...
        try {
            ...
            jdbc.update("""
                ...
                """,
                    ...
                    java.sql.Timestamp.from(Instant.now()), LocalDate.now(), strategyName);
            recordEntry(symbol, strategyName, "PNL", realisedPnl,
                    "Trade closed — " + (isWin ? "WIN" : "LOSS/BREAKEVEN"));
            log.info("[AI-LEDGER] Exit recorded: {} P&L=₹{} ({}) — margin released ₹{} (strategy={})",
    ...
    private void recordEntry(String symbol, String strategyName, String type,
        ...
        try {
            jdbc.update("""
                ...
                VALUES (?,?,?,?,?,?,?)
                """,
                    LocalDate.now(), symbol, strategyName, type,
    ...
    public BigDecimal getTodayRealisedPnl(String strategyName) {
        try {
            Object pnl = jdbc.queryForObject(
                    ...
                    BigDecimal.class, LocalDate.now(), strategyName);
